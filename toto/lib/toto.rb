...
module Toto
  ...
  class Article < Hash
    ...
    def path
      return "/#{@config[:prefix]}#{self[:date].strftime("/#{@config[:dateformat]}/#{slug}#{@config[:suffix]}")}".squeeze('/')
    end
  ...
  class Categories < Array
    ...
    def cloud
      @articles.map { |article|  article.category.to_s.split('/') }.flatten.uniq.sort
    end
